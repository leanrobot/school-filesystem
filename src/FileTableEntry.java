public class FileTableEntry {  // Each table entry should have
    ...
    public int read(byte[] buffer){
    	...
    	int sizeLeft = inode.length - seekPtr;
    	int bytesRead = 0;
    	
    	if (sizeLeft > bufferLength){
    		//read bufferLength from file into buffer
    		//set bytesRead
    		bytesRead = bufferLength;
    		//if error, return here
    	}
    	else{ //<= bufferLength
    		//read sizeLeft bytes into buffer
    		//set bytesRead
    		bytesRead = sizeLeft;
    		//if error, return here
    	}
    	
    	//TODO: Need to clamp to inode size or will the above have taken care of that?
